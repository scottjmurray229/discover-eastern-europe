...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'castle': ['prague', 'krakow', 'budapest'],
    'thermal': ['budapest', 'bratislava', 'ljubljana'],
    'nightlife': ['budapest', 'prague', 'belgrade'],
    'architecture': ['prague', 'riga', 'budapest'],
    'history': ['krakow', 'prague', 'dubrovnik'],
    'nature': ['ljubljana', 'split', 'sofia'],
    'food': ['budapest', 'krakow', 'belgrade'],
    'wine': ['budapest', 'bucharest', 'split'],
    'budget': ['sofia', 'bucharest', 'belgrade'],
    'backpacker': ['budapest', 'prague', 'sofia'],
    'beach': ['dubrovnik', 'split', 'tallinn'],
    'art-nouveau': ['riga', 'prague', 'budapest'],
    'medieval': ['tallinn', 'dubrovnik', 'krakow'],
    'romantic': ['prague', 'dubrovnik', 'budapest'],
    'culture': ['prague', 'krakow', 'vilnius'],
    'hiking': ['ljubljana', 'sofia', 'split'],
    'central-europe': ['prague', 'budapest', 'bratislava'],
    'balkans': ['dubrovnik', 'split', 'belgrade'],
...
function buildSystemPrompt(): string {
  return `You are the Discover Eastern Europe AI Trip Planner. You are an expert on travel across Eastern Europe — from the cobblestone streets of Prague to the Baltic coastline of Tallinn.

RULES:
- All prices in BOTH EUR (€) and USD ($). Use rate: €1 = $1.08 USD
- Include specific restaurant names, hotel recommendations, and transport details (trains, buses, flights)
...
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-prague"
- Know that some countries use EUR (Slovakia, Estonia, Latvia, Lithuania, Slovenia, Croatia) while others use local currencies (Czech koruna, Hungarian forint, Polish zloty, Romanian leu, Bulgarian lev, Serbian dinar) — but always express prices in EUR for consistency
...
{
  "title": "string — trip title like '7-Day Central Europe Circuit'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "eur": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Prague'",
      "destination": "string — slug like 'prague'",
      "items": [
        {
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Eastern Europe trip:\n${parts.join('\n')}`;
